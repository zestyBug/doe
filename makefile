...
headerCutil=$(cutilDir)/advancedArray.h $(cutilDir)/advancedQueue.hpp $(cutilDir)/aStack.hpp $(cutilDir)/bitset.hpp $(cutilDir)/gc_ptr.hpp $(cutilDir)/HashHelper.hpp $(cutilDir)/prototype.hpp $(cutilDir)/QueueQueue.hpp $(cutilDir)/range.hpp $(cutilDir)/semaphore.hpp $(cutilDir)/SmallVector.hpp $(cutilDir)/span.hpp $(cutilDir)/StaticArray.hpp $(cutilDir)/string_view.hpp $(cutilDir)/unique_ptr.hpp $(cutilDir)/basics.hpp
headerInclude=$(includeDir)/ECS/Archetype.hpp $(includeDir)/ECS/ArchetypeVersionManager.hpp $(includeDir)/ECS/defs.hpp $(includeDir)/ECS/EntityCommandBuffer.hpp $(includeDir)/ECS/EntityComponentManager.hpp
...
$(BIN_DEBUG)/job: $(OBJ_DEBUG)/test/job.o
	mkdir -p $(@D)
	$(CCPP) -o $@ $^ $(CFLAG_debug)
