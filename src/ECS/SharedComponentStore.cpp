...
using namespace ECS;
void SharedComponentStore::initiate(uint32_t typeIndex){
...
void SharedComponentStore::resize(uint32_t typeIndex){
    const uint32_t typeSize = TypeManager::GetTypeInfoPointer()[typeIndex].TypeSize;
    SharedComponentChunk* ptr = dataChunk[typeIndex].get();
    if(unlikely(!ptr)){
        initiate(typeIndex);
...
void *SharedComponentStore::getPointer(SharedComponentIndex index) noexcept {
    if(index.isNull())
        return nullptr;
...
SharedComponentIndex SharedComponentStore::insert(TypeID type,void *data) {
...
SharedComponentIndex SharedComponentStore::getDefaultValue(TypeID type){
    if(!type.isSharedComponent() || type.isZeroSized())
        throw std::bad_typeid();
    const uint32_t typeIndex = type.index();
    initiate(typeIndex);
...
void SharedComponentStore::addReference(SharedComponentIndex index, uint32_t num) {
    const uint32_t typeIndex = getTypeIndex(index);
    const uint32_t elementIndex = getElementIndex(index);
    SharedComponentChunk* ptr = dataChunk[typeIndex].get();
    if(!ptr || elementIndex >= ptr->count)
...
void SharedComponentStore::removeReference(SharedComponentIndex index, uint32_t num) {
    const uint32_t typeIndex = getTypeIndex(index);
    const uint32_t elementIndex = getElementIndex(index);
    SharedComponentChunk* ptr = dataChunk[typeIndex].get();
    if(!ptr || elementIndex >= ptr->count)
...
void SharedComponentStore::incrementVersion(SharedComponentIndex index) {
    const uint32_t typeIndex = getTypeIndex(index);
    const uint32_t elementIndex = getElementIndex(index);
    SharedComponentChunk* ptr = dataChunk[typeIndex].get();
    if(!ptr || elementIndex >= ptr->count)
...
uint32_t SharedComponentStore::getVersion(SharedComponentIndex index) const {
    const uint32_t typeIndex = getTypeIndex(index);
    const uint32_t elementIndex = getElementIndex(index);
...
void SharedComponentStore::checkInternalConsistency()
{
    for (uint32_t i=0;i<Constants::MaximumTypesCount;i++)
    {
        SharedComponentChunk *ptr = dataChunk[i].get();
